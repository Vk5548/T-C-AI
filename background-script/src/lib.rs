...
// Rc: A reference-counted smart pointer. It enables multiple ownership of the same data.
// RefCell: Allows for interior mutability, meaning that you can mutate data even when it is shared between multiple owners via Rc.
// HashMap: A data structure to store key-value pairs.

use gloo_console as console;
// A Rust crate that provides bindings to the browser's console for logging.

use gloo_timers::future::TimeoutFuture;
//Provides a way to create asynchronous delays in Rust, mimicking JavaScript's setTimeout.

use gloo_utils::format::JsValueSerdeExt;
// Provides utilities for serializing and deserializing Rust types to and from JavaScript JsValue.

use js_sys::{Function, Object};
// Part of Rust's standard library for JavaScript interoperability, providing access to JavaScript types.
...
use serde::Serialize;
// A serialization/deserialization framework for Rust.
// It's used to convert Rust data structures into formats like JSON that can be used in JavaScript.

use thiserror::Error;
// A crate for defining error types in Rust with helpful error messages.

use wasm_bindgen::{prelude::*, JsCast};
// A Rust library that facilitates interaction between Rust and JavaScript.
// The #[wasm_bindgen] attribute is used to expose Rust functions and types to JavaScript.

use web_extensions_sys::{chrome, Port, Tab, TabChangeInfo};
// These are bindings to the Chrome WebExtensions API, allowing you to interact with browser features like tabs and ports.

const VERSION: &str = env!("CARGO_PKG_VERSION");
// A constant that holds the current version of the crate, pulled from the Cargo package configuration.
...
// PortContext and ConnectedPorts: These structs manage the state of communication ports.
// PortContext tracks an individual port and its last request ID.
...
// ConnectedPorts : It manages a collection of active ports
...
// PortError: An enum that defines possible errors related to port management, like attempting to send a message to a disconnected port.
...
impl ConnectedPorts {
    ...
    // post_message_js and post_message<T: Serialize>:
    // Methods that send messages over the ports. One handles raw JavaScript values (JsValue),
    // and the other handles serializable Rust types.
...
// App: The main struct representing the application, managing overall state including connected ports and request IDs.
...
// start() Function:
// The entry point when the background script is started. It sets up event listeners
// for message passing, tab changes, and port connections.
#[wasm_bindgen]
pub fn start() {
    ...
    // on_message, on_tab_changed, on_connect_port, and on_port_message Functions:
    // Handle various events that occur in the browser, like receiving messages,
    // detecting tab changes, or managing port connections and disconnections.
    let on_message = {
...
// StreamingTask and StreamingTaskStatus:
// Manage tasks that stream data asynchronously. This is especially relevant in web extensions
// where data might be fetched or processed in chunks over time.
...
// handle_app_request and handle_port_request:
// Process incoming requests, potentially generating responses. These functions are central to the application's logic,
// determining how to respond to various types of requests from the extension's frontend or other parts.
